...
const GithubProvider = ({ children }) => {
  ...
  const searchGithubUsers = async (user, params = "") => {
    ...
    if (response) {
      let githubLink = response.headers["link"]
        ? response.headers["link"].split(",").map((lnk) => {
  ...
  useEffect(() => {
    ...
    return () =>
      console.log(
        "Thanks for checking: please visit for code https://github.com/kedarvijaykulkarni/github-search-react"
      );
  }, []);
